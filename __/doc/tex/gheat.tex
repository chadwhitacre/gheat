...
Be sure to tweak the variables under \code{GCopyright} and \code{getTileUrl}
for your situation. This code should run when the page loads, and then when the
map is loaded, you need to add the overlay thus defined to the map, like this:

\begin{verbatim}
map.addOverlay(new GTileLayerOverlay(gheat));
\end{verbatim}

If it works, you will see the heatmap tiles loaded overtop of the underlying
Google Map, and you will feel pumped.
